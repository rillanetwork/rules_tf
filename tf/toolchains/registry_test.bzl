...
These run at analysis time and reach no registry: only the HCL subset parser is
covered, not the lookup that decides which file to read.
"""

load("@bazel_skylib//lib:unittest.bzl", "asserts", "unittest")
load(":registry.bzl", "hcl_credentials_token")
...
def registry_test_suite(name = "registry_test"):
    """Declares the registry credential tests.
