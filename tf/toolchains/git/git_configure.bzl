...
def _write_build(rctx, path, workspace_dir):
    ...
    rctx.template(
        "BUILD",
        Label("//tf/toolchains/git:BUILD.tpl"),
        substitutions = {
            "{GENERATOR}": "@rules_tf//tf/toolchains/git/git_configure.bzl%find_system_git",
            "{GIT_PATH}": str(path),
...
def experimental_find_system_git(name, workspace_file = None, verbose = False):
    ...
    native.register_toolchains(
        ...
        "@rules_tf//tf/toolchains/git:git_missing_toolchain",
    )
