...
Run at analysis time and fetch nothing. The documents below are excerpts of
real releases, one per tool: each spells its archive name differently.
"""
...
# A ruleset's checksums list the bare binary next to its archive; the binary's
# line is the archive's line with ".zip" removed.
_RULESET_CHECKSUMS = """
...
def _archive_naming_test_impl(ctx):
    ...
    # Each tool spells its archive name differently: version in the name or not,
    # dash- or underscore-joined platform.
...
def _lookup_test_impl(ctx):
    ...
    # A platform absent from the release returns None, not an error.
...
def _suffix_ambiguity_test_impl(ctx):
    ...
    # Matching by suffix must not return the bare binary's hash when the
    # archive is asked for, whichever order the document lists them in.
    asserts.equals(
        env,
