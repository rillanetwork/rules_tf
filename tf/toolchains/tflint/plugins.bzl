...
# tflint installs rulesets only from GitHub releases.
...
def _strip_comments(line, in_block):
    """Line with its `#`, `//` and `/* */` comments removed, and whether a block comment is still open.

    A comment marker inside a quoted string is content, not a comment.
    """
    code = []
    quoted = False
    skip = False
    for i in range(len(line)):
        ...
        if c == "#" or pair == "//":
            break
        if pair == "/*":
            # A block comment separates tokens, as whitespace would.
            code.append(" ")
            in_block = True
            skip = True
            continue
...
def _heredoc_terminator(line):
    """The terminator a heredoc opens on line, or None."""
    if "<<" not in line:
        return None
...
def parse_tflint_plugins(config):
    """The downloadable rulesets, `plugin` blocks with a `source`, a tflint config declares.
    ...
    for raw in config.splitlines():
        ...
        # Comments go before anything else reads the line, so a brace, quote or
        # `<<` inside one cannot count. A heredoc body is content and is never stripped.
    ...
    for p in plugins:
        ...
        if not p["version"]:
            fail("tflint plugin \"%s\" has a source but no version; pin one." % p["name"])

        parts = p["source"].split("/")
        if len(parts) != 3 or parts[0] != _PLUGIN_HOST:
            fail("tflint plugin \"%s\" has source '%s'; it must be '%s/<owner>/<repo>'." % (
                p["name"],
                p["source"],
                _PLUGIN_HOST,
            ))
...
def resolve_tflint_plugins(ctx, plugins, os, arch, facts):
    ...
    Args:
      ...
      plugins: rulesets from `parse_tflint_plugins`.
...
def warn_unverified_plugins(plugins):
    """Warns that the given rulesets were pinned without a signature check.
    ...
    print(("rules_tf: tflint ruleset(s) pinned without a signature check, from checksums.txt " +
           "only: %s") % names)  # buildifier: disable=print

def _file_sha256(ctx, path, output):
    """The sha256 of a local file, or None.

    Starlark has no hashing, so this borrows `download`'s via a `file://` URL.
    """
...
def verify_tflint_plugins(ctx, tflint, config, plugins, facts, platforms, workdir):
    ...
    Args:
      ctx: the module extension's `module_ctx`.
      tflint: path of the tflint binary.
    ...
    # Refused up front, since `--init` installs these with only a warning.
    for p in plugins:
        if p["source"].split("/")[1] == _OFFICIAL_OWNER or p["signing_key"]:
            verifiable.append(p)
        else:
            errors.append(
                ("tflint ruleset %s %s needs a signing_key: tflint only knows the key for %s. " +
                 "Add one to the plugin block, or set tflint_plugin_verification = \"off\" on " +
                 "the tf.download tag to skip the check.") % (
                    p["source"],
                    p["version"],
                    _OFFICIAL_OWNER,
                ),
    ...
    if result.return_code != 0:
        return errors + [
            ("could not verify the tflint ruleset(s) %s: `tflint --init` exited %d. Check " +
             "network access and that each signing_key is the publisher's.\n%s") % (
    ...
    for r in pending:
        ...
        if not installed.exists:
            errors.append(
                "`tflint --init` installed no %s for ruleset %s %s." % (
                    binary,
                    p["source"],
                    p["version"],
                ),
        ...
        if not ctx.download_and_extract(
            ...
            errors.append(
                ("the recorded sha256 for tflint ruleset %s %s does not match %s: either the " +
                 "hash is stale or the release was replaced. Find out which before relocking.") % (
                    p["source"],
                    p["version"],
                    p["download_url"],
                ),
        ...
        if pinned_sha256 == None or installed_sha256 == None:
            errors.append(
                "could not hash the %s of tflint ruleset %s %s." % (
                    binary,
                    p["source"],
                    p["version"],
                ),
        ...
        if pinned_sha256 != installed_sha256:
            errors.append(
                ("tflint ruleset %s %s does not match the release tflint verified: the " +
                 "pinned %s has sha256 %s, the verified one %s.") % (
                    p["source"],
                    p["version"],
                    binary,
                    pinned_sha256,
        ...
        if contradicted:
            errors.append(
                ("tflint ruleset %s %s has recorded hashes that the signed %s contradicts, " +
                 "for: %s. Find out where they came from before relocking.") % (
                    p["source"],
                    p["version"],
                    r["url"],
                    ", ".join(contradicted),
...
def download_tflint_plugins(ctx, plugins, plugin_dir):
    ...
    # The BUILD template exports the directory even when it is empty.
    if len(plugins) == 0:
    ...
    for p in plugins:
        ...
        # Path tflint looks a plugin up under: source and version verbatim.
