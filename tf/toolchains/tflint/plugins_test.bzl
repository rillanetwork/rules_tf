...
Run at analysis time and reach no release API. Cover the config grammar and
the narrowing that decides what still needs verifying.
"""
...
def _unverified_plugins_test_impl(ctx):
    ...
    # No mark on any platform is also unverified.
    asserts.equals(env, [plugin], unverified_plugins(
        _facts(plugin, []),
        [plugin],
        _PLATFORMS,
...
_inline_and_block_comments_test = unittest.make(_inline_and_block_comments_test_impl)
...
def tflint_plugins_test_suite():
    ...
    unittest.suite(
        ...
        _inline_and_block_comments_test,
