...
Most of tflint's rules live in ruleset plugins rather than in the binary, and a plugin is downloaded from its own
GitHub release. This page covers declaring those rulesets so the toolchain mirrors them, and what pins them.

For the per-module `tflint_config` attribute on the lint rules themselves, see the
[README](../README.md#using-tf-rules).
