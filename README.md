...
`backend` takes a single-key dict of `{type: config}` and renders a `*.bazel.backend.tf.generated` file; `tfvars` (and label-keyed `tfvars_deps`) render a `*.bazel.auto.tfvars.generated` file. Pass `output_json = True` to also write `plan.tfplan.json` next to the binary plan.
