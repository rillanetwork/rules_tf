...
A root module is a `tf_module` with `root = True`, which adds the Terraform lifecycle targets (absorbed from the former `rules_tf_apply` module) alongside the module's own sources and checks. This effectively enables using tools like bazel-diff to selectively apply changes to Terraform modules only when necessary.
...
`backend` is required with `root = True`; the other root-only arguments (`tfvars`, `tfvars_deps`, `output_json`) are rejected without it. `backend` takes a single-key dict of `{type: config}` and renders a `*.bazel.backend.tf.json.generated` file; `tfvars` (and label-keyed `tfvars_deps`) render a `*.bazel.auto.tfvars.json.generated` file. Pass `output_json = True` to also write `plan.tfplan.json` next to the binary plan.
...
### Root Modules in Another Package

When the root module's sources live in a different package to the one being declared, the `tf_root_module` macro declares the same lifecycle targets against a `tf_module` label elsewhere:
...
`tf_module(root = True)` is the same call with `module` pointing at itself, so both spellings produce identical targets and `@rules_tf//tools:list_modules` enumerates them the same way.
...
For terraform subcommands without a dedicated target (`state`, `import`, `taint`, `output`, `refresh`, `show`, ...), a root module exposes a generic `<name>.tf` target that forwards all arguments to `terraform -chdir=<module>`:
...
Enumerates root module targets (via `kind(tf_plan, <query_path>)`) and emits a JSON matrix describing each one:
