...
## Planning and Applying Tf Root Modules
...
It can be paired with a `tf_module` in the same `BUILD.bazel` file (as below), or it can reference a tf_module label from elsewhere in your workspace.
...
Running these generates a `bazel-tf` directory at the root of the workspace that should be git-ignored. This directory
contains the Terraform state and plan files generated by each of the phases.

The `module` property of `tf_root_module` can also point at a `tf_module` in another package, allowing you to define a
root module that uses a shared module from elsewhere in your workspace. In this way, you can define reusable "stack"
modules, then expose them into different environments, each with its own backend and tfvars.
