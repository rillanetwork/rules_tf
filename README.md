...
The backend `config` is written the same way, as a `*.backend.tf.json` file in Terraform's JSON configuration syntax, so it too carries typed values. A nested map is how a backend's nested blocks are written:
...
Running these generates a `bazel-tf` directory at the root of the workspace that must be gitignored. This directory contains the Terraform state and plan files generated by each of the phases, in a directory of its own for each root module: `bazel-tf/<package>/<name>/`, so `bazel-tf/path/to/vpc/` above.

`backend` takes a single-key dict of `{type: config}` and renders a `*.bazel.backend.tf.json.generated` file; `tfvars` (and label-keyed `tfvars_deps`) render a `*.bazel.auto.tfvars.json.generated` file. Pass `output_json = True` to also write `plan.tfplan.json` next to the binary plan.

Because `module` can name a `tf_module` in another package, one module can back any number of roots, typically one per environment, each with its own backend and tfvars. Every root keeps its own `.terraform` and plan under `bazel-tf/<package>/<name>/`, so initializing or planning one never disturbs another, including when `@rules_tf//tools:run` works through them in parallel.

### Destroying a Module

`<name>.destroy` mirrors `.plan` - it runs `terraform plan -destroy` and writes the destroy plan to `bazel-tf/<package>/<name>/plan.tfplan`. The subsequent `<name>.apply` then consumes that plan exactly like a forward apply, preserving the same review-then-execute workflow:

```bash
bazel run //path/to:vpc.init
bazel run //path/to:vpc.destroy   # writes a destroy plan to bazel-tf/path/to/vpc/plan.tfplan
...
Each row: `{"package", "module_package", "state_dir", "name", "skip", "affected"}`.
...
- `module_package` is the package holding the terraform sources - equal to `package` unless the root points `module =` at a shared module elsewhere.
- `state_dir` is the root's directory under `bazel-tf/` (`<package>/<name>`), holding its `.terraform` and `plan.tfplan`. Read a root's plan from here: unlike `module_package`, it is unique to each root.
